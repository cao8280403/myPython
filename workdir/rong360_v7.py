...
html = 'http://www.caifuchaoshi.cn/loan'
# 定义主流的标签，用于后面匹配
...
def insert_sql(node_prefix, node_brother_count, node_likely_count, imgcount, julilist, cengshu, nodepath, brother_sort,
               totalstrcount, strcount, numcount, countofnonzerokeywords, numberrate, isproduct):
    sql = """INSERT INTO node_list419(guid,create_time, website, node_prefix, node_brother_count, node_likely_brother_count,node_depth,node_img,node_to_img,node_total_depth,
            nodepath,isproduct, node_brother_sort,totalstrcount,strcountdict,numcountdict,countofnonzerokeywords,numberrate)
    ...
    try:
        ...
        cursor.execute(sql, (
            ...
            nodepath, isproduct, brother_sort, totalstrcount, str(strcount), str(numcount), countofnonzerokeywords,
            numberrate))
        # 提交到数据库执行
        db.commit()
...
def setmaxcengshu(maxcengshu):
    sql = """UPDATE node_list419 SET node_total_depth = %s where website = %s """
    try:
        # 执行sql语句,这种写法防止注入
...
# 获取此节点所有叶子节点关键字个数不为0的个数
def getcountofnonzerokeywords(dict):
    count = 0
    for i in dict:
        if dict[i] > 0:
            count += 1
...
# 获取此节点所有叶子节点数字出现率
def getnumberrate(dict, totalstrcount):
    count = 0
    for i in dict:
        count += dict[i]
    count = count / totalstrcount
    return count
    ...
    if (isinstance(soup, BeautifulSoup) | isinstance(soup, Tag)):  # 只解析有用的节点 tag标签
        ...
        if len(nodes) != 0:
            ...
            for i in nodes:
                ...
                # print("ccccccccc"+totalstr)
    elif (isinstance(soup, NavigableString)):  # 叶子节点
        tempsoup = soup.replace(' ', '')
        tempsoup = tempsoup.replace('\n', '')

    totalstr = tempsoup + totalstr
    if len(totalstr) > 0:
        # print("aaaaaaaa"+node_prefix)
        # print("bbbbbbbb"+totalstr)
        totalstrcount = len(totalstr)
        strcount = getstrdict(totalstr)
        numcount = getnumdict(totalstr)
        countofnonzerokeywords = getcountofnonzerokeywords(strcount)
        numberrate = getnumberrate(numcount, totalstrcount)
    if len(node_prefix) > 0:
        insert_sql(node_prefix, brothernodescount, likenodecount, imgcount, str(julilist), cengshu, nodepath, brother_sort,
               totalstr, strcount, numcount, countofnonzerokeywords, numberrate, "0")
    return totalstr
