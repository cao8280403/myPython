...
def insert_sql(node_prefix, node_brother_count, node_likely_count, imgcount, julilist, cengshu, nodepath, brother_sort,
    ...
    sql = """INSERT INTO node_list410(guid,
...
def setmaxcengshu(maxcengshu):
    sql = """UPDATE node_list410 SET node_total_depth = %s where website = %s """
    try:
        # 执行sql语句,这种写法防止注入
...
def analyzedom(soup):
    ...
    if (likenodecount > 0):
        totalstrcount = len(str(soup))
        strcount = getstrdict(soup)
        numcount = getnumdict(soup)
