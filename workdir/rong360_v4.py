...
def insert_sql(node_prefix, node_brother_count, node_likely_count, imgcount, julilist, cengshu, nodepath, brother_sort, isproduct):
    sql = """INSERT INTO node_list(guid,
             create_time, website, node_prefix, node_brother_count, node_likely_brother_count,node_depth,node_img,node_to_img,node_total_depth,nodepath,isproduct, node_brother_sort)
    ...
    try:
        # 执行sql语句,这种写法防止注入
        cursor.execute(sql, (guid, createtime, html, node_prefix, node_brother_count, node_likely_count, cengshu, imgcount, julilist, 1, nodepath, isproduct, brother_sort))
        # 提交到数据库执行
        db.commit()
    except Exception:
        print(Exception)
...
def analyzedom(soup):
    ...
    insert_sql(node_prefix, brothernodescount, likenodecount, imgcount, str(julilist), cengshu, nodepath, brother_sort, "0")
